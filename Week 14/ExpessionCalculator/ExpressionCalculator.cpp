...
ExpressionCalculator::BooleanExpr* parse(const StringView& str)
{
	...
	StringView strWithoutBrackets = str.substr(1, str.length() - 2); //We remove the brackets.

	
	int countBrackets = 0;
	for (size_t i = 0; i < strWithoutBrackets.length(); i++) //we start searching for the operation.
	{														 // then we split to two expressions
		...
		else if (isOperator(strWithoutBrackets[i]) && countBrackets == 0)
		{
			if (strWithoutBrackets[i] == NEG)
				return new ExpressionCalculator::UnaryOperation('!', parse(strWithoutBrackets.substr(i + 1, strWithoutBrackets.length() - 1)));
			else
				return new ExpressionCalculator::BinaryOperation(parse(strWithoutBrackets.substr(0, i)), strWithoutBrackets[i], parse(strWithoutBrackets.substr(i + 1, strWithoutBrackets.length() - (i + 1))));
		}
